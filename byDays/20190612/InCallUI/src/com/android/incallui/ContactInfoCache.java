...
public class ContactInfoCache implements ContactsAsyncHelper.OnImageLoadCompleteListener {
    ...
    private void findInfoQueryComplete(CallIdentification identification,
        ...
        if(didLocalLookup && SprdUtils.UNIVERSE_UI_SUPPORT
                 && TextUtils.isEmpty(cacheEntry.name)
                 && !callerInfo.isEmergencyNumber()
                 && !callerInfo.isVoiceMailNumber()){
            cacheEntry.name = mContext.getString(R.string.unknown_contacts);
            cacheEntry.isRealNoName = true;
        } else{
            cacheEntry.isRealNoName = false;
        }
        /* @} */
    ...
    /* @} */
    public void dump() {
        for (int key : mInfoMap.keySet()) {
            Log.d(TAG, "dump ContactInfoMap: \n");
            Log.d(TAG, "key: "+key+"; value: "+mInfoMap.get(key).toString());
        }
    }
    ...
    private ContactCacheEntry buildEntry(Context context, int callId,
        ...
        if (info.photoResource != 0) {
            ...
            if (info.cachedPhoto != null) {
                ...
                photo = context.getResources().getDrawable(photoResId);
            }
    ...
    public static void populateCacheEntry(Context context, CallerInfo info, ContactCacheEntry cce,
        ...
        boolean isSipCall = false;
            ...
            if (isIncoming) {
                ...
                // place for incoming calls.
                displayLocation = info.geoDescription; // may be null
                Log.d(TAG, "  ==>  displayLocation: " + displayLocation);
            }
            /* @} */
    ...
    public static class ContactCacheEntry {
        ...
        public boolean isRealNoName;//SPRD:add for unknown contact if name doesn't exist contact
