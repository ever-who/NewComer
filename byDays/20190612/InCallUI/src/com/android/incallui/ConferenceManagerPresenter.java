...
public class ConferenceManagerPresenter
    ...
    private void updatePikel(CallList callList) {
        final Call currentCall = callList.getActiveOrBackgroundCall();
        if (currentCall == null) {
            return;
        }
        ArrayList<Call> calls = new ArrayList<Call>(currentCall.getChildCallIds().size());
        for (int callerId : currentCall.getChildCallIds()) {
            calls.add(callList.getCall(callerId));
        }

        Log.d(this, "updatePikel Number of calls is " + String.valueOf(calls.size()));
    ...
    public void separateConferenceConnectionPikel(int callId) {
        CallCommandClient.getInstance().separateCall(callId);
    }

    public void endConferenceConnectionPikel(int callId) {
        CallCommandClient.getInstance().disconnectCall(callId);
    }
