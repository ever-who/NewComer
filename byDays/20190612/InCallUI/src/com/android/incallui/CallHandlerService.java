...
import com.android.internal.telephony.gsm.SuppServiceNotification;  //SPRD: bug 845977
...
public class CallHandlerService extends Service {
    ...
    /* SPRD: add for fail call back, special for suppServiceFailed @{ */
    private static final int ON_SUPPORT_CODE = 11;
    /* @} */
    private static final int ON_PREPARE_UI = 12;// SPRD: modify for performance optimization
    private static final int ON_SPRD_CALL_COMMANDS = 13;
    private static final int ON_RINGTONE_PLAYING = 14;//SPRD:add for Bug348353
    private static final int LARGEST_MSG_ID = /*ON_DESTROY*/ON_RINGTONE_PLAYING;
    private static final int ON_HOLD_CALL_MT = 15; //SPRD: bug 845977
    ...
    public void onCreate() {
        Log.i(TAG, "onCreate");
        super.onCreate();
        /* SPRD: add for Bug286937@ { */
        ActivityManager am = (ActivityManager) getSystemService(Context.ACTIVITY_SERVICE);
        am.setSelfProtectStatus(ActivityManager.PROCESS_STATUS_PERSISTENT);
        /* @} */
    ...
    private final ICallHandlerService.Stub mBinder = new ICallHandlerService.Stub() {
        ...
        public void onError(int code, String msg){
            mMainHandler.sendMessage(mMainHandler.obtainMessage(ON_SUPPORT_CODE, code, 0, msg));
        }
        /* @} */
        ...
        public void onPrepareUi(){
            try {
                Log.i(TAG, " callhandleservice :onPrepareUi");
                mMainHandler.sendMessage(mMainHandler.obtainMessage(ON_PREPARE_UI, null));
            } catch (Exception e) {
                Log.e(TAG, "Error processing onPrepareUi() call.", e);
            }
        }
        ...
        public void onRingtonePlaying(){
            try {
                Log.i(TAG, " callhandleservice :onRingtonePlaying");
                mMainHandler.sendMessage(mMainHandler.obtainMessage(ON_RINGTONE_PLAYING, null));
            } catch (Exception e) {
                Log.e(TAG, "Error processing onRingtonePlaying() call.", e);
            }
        }
        /* @} */
        ...
        public void startSprdCallService(ISprdCallCommandService service) {
            try {
                ...
                mMainHandler.sendMessage(mMainHandler.obtainMessage(ON_SPRD_CALL_COMMANDS, service));
            } catch (Exception e) {
        ...
        @Override
          public void onMTCallHold(int onhold) {
            try {
                Log.i(TAG, "onMTCallHold: onhold" + onhold);
                mMainHandler.sendMessage(mMainHandler.obtainMessage(ON_HOLD_CALL_MT, onhold));
            } catch (Exception e) {
                Log.e(TAG, "Error processing onUpdate() call.", e);
            }
        }
    ...
    private void executeMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case ON_DESTROY:
                doStop();
                break;
            case ON_SUPPORT_CODE:
                if (mInCallPresenter != null) {
                    mInCallPresenter.onError(msg.arg1,
                            msg.obj == null ? null : String.valueOf(msg.obj));
                }
                break;
            /* SPRD: modify for performance optimization @{ */
            case ON_PREPARE_UI:
                Log.i(TAG, "ON_PREPARE_UI");
                mCallList.onPrepareUi();
                break;
            /* @} */
            /* SPRD: modify for Bug348353 @{ */
            case ON_RINGTONE_PLAYING:
                Log.i(TAG, "ON_RINGTONE_PLAYING");
                mInCallPresenter.updateIncomingMuteButton();
                break;
            /* @} */
            case ON_SPRD_CALL_COMMANDS:
                ISprdCallCommandService service = (ISprdCallCommandService) msg.obj;
                CallCommandClient.getInstance().setSprdService(service);
                break;
            case ON_HOLD_CALL_MT:   //SPRD: bug 845977
                if(mCallList != null ) {
                    Call activeCall = mCallList.getActiveCall();
                    if(activeCall == null) {
                        Log.d(this, " ON_HOLD_CALL_MT return as call is null" );
                        return;
                    }

                    switch(Integer.valueOf(String.valueOf(msg.obj))) {
                        case SuppServiceNotification.MT_CODE_CALL_ON_HOLD:
                            activeCall.setMTCallHold(true);
                            Log.d(this, "<hold> ON_HOLD_CALL_MT setMTCallHold as true");
                            break;

                        case SuppServiceNotification.MT_CODE_CALL_RETRIEVED:
                            activeCall.setMTCallHold(false);
                            Log.d(this, "<retrive> ON_HOLD_CALL_MT csetMTCallHold as false ");
                            break;
                    ...
                    if (mInCallPresenter != null) {
                        mInCallPresenter.onMTCallHold(Integer.valueOf(String.valueOf(msg.obj)));
                    }
                }
                break;
            default:
