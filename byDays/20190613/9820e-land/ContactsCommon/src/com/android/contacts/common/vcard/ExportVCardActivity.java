...
public class ExportVCardActivity extends Activity implements ServiceConnection,
    ...
    private class ExportConfirmationListener implements DialogInterface.OnClickListener {
        ...
        public void onClick(DialogInterface dialog, int which) {
            if (which == DialogInterface.BUTTON_POSITIVE) {
                ...
                /**
                * SPRD:
                *   fix bug2536 add support for 'selective export to SD card'
                *
                ...
                * @{
                */
                final ExportRequest request = new ExportRequest(mDestinationUri,mSelectedContacts);
                /**
                * @}
                */
    ...
    protected void onCreate(Bundle bundle) {
        ...
        /**
        * SPRD:
        *   fix bug103821 Unable to export the contacts to external memory card
        *
        * Original Android code:
        ...
        */
        ...
        mDeviceType = getIntent().getIntExtra("device_type", -1);
        if (Constants.STORAGETYPE == 0) {
            targetDirectory = Environment.getExternalStoragePath();
        } else {
            if (mDeviceType == R.string.export_to_sdcard) {
                targetDirectory = Environment.getExternalStoragePath();
            } else {
        ...
        */
        ...
        /**
        * SPRD:
        *   fix bug143117 Export vcard failed when sd card storage is not enough
        *
        * Original Android code:
        * 
        * 
        * @{
        */
        if(getSdCardAvailableSize(targetDirectory) < MIN_SD_STORAGE_SIZE){
            showDialog(R.id.dialog_sdcard_storage_not_enough);
            return;
        }
        /**
        * @}
        */
         ...
         * @{
         */
        mSelectedContacts=getIntent().getStringArrayListExtra("lookup_keys");
         /**
         * @}
         */
         ...
         * @{
         */
        intent.putExtra("targetDirectory", targetDirectory.toString());
        /**
         * @}
         */
    ...
    protected Dialog onCreateDialog(int id, Bundle bundle) {
        switch (id) {
            ...
            * @{
            */
            case R.id.dialog_sdcard_storage_not_enough:{
                mProcessOngoing = false;
                return new AlertDialog.Builder(this)
                .setIconAttribute(android.R.attr.alertDialogIcon)
                .setMessage(R.string.sdcard_storage_not_enough)
                /*
                * SPRD:bug 274332 T card full, export vcard to T card , after touch select ok button ,then touch screen ,ANR
                *
                * @{
                */
                .setCancelable(false)
    ...
    protected void onStop() {
        ...
        if (!isFinishing()) {
            /**
            * SPRD:
            *   fix bug138337 Pop-up dialog box to export contacts to switch the screen anyway, the dialog box disappears, and lock the keyboard will disappear
            *
            * Original Android code:
            * unbindAndFinish();
            * 
    ...
    * @{
    */
    private static final long MIN_SD_STORAGE_SIZE = 1024;// the minimum storage of SD cards 1024 kb
    private int mDeviceType=-1;
    private ArrayList<String> mSelectedContacts;

    private long getSdCardAvailableSize(File file){
