...
public class ContactLoader extends AsyncTaskLoader<Contact> {
    ...
    private Contact loadContactHeaderData(final Cursor cursor, Uri contactUri) {
        ...
        final String sync1 = cursor.getString(ContactQuery.SYNC1);
        final String fdnStr = cursor.getString(ContactQuery.SYNC2);
        boolean isFdn = false;
        int fdnPhoneId = -1;
        if (fdnStr != null && fdnStr != "" && fdnStr.startsWith("fdn")) {
            isFdn = true;
            fdnPhoneId = Integer.parseInt(fdnStr.substring(3));
        }
        return new Contact(mRequestedUri, contactUri, lookupUri, directoryId, lookupKey,
                contactId, nameRawContactId, displayNameSource, photoId, photoUri, displayName,
                altDisplayName, phoneticName, starred, presence, sendToVoicemail,
                customRingtone, isUserProfile, displayAccountName, displayAccountType, "sdn".equals(sync1),
                isFdn, fdnPhoneId);
        /**
         * @}
         */
