...
public abstract class ContactListAdapter extends ContactEntryListAdapter {
    ...
    protected View newView(Context context, int partition, Cursor cursor, int position,
            ViewGroup parent) {
        Log.d("hjc_adapter", "newView() called with: context = [" + context + "], partition = [" + partition + "], cursor = [" + cursor + "], position = [" + position + "], parent = [" + parent + "]");
//        if(position==0){
...
//        }
    ...
    protected void bindSimPhoto(final ContactListItemView view, int partitionIndex, Cursor cursor) {
            String accountType=cursor.getString(ContactQuery.CONTACT_DISPLAY_ACCOUNT_TYPE);
            String accountName=cursor.getString(ContactQuery.CONTACT_DISPLAY_ACCOUNT_NAME);
            if (accountType != null
                    && (accountType
                ...
                int phoneId = 0;
				Drawable drawable = null;
				if(accountName != null && accountName.equals("SIM2")) {
					phoneId = 1;	
				}
				drawable = AccountTypeManager.getInstance(mContext).getListFdnIcon(phoneId);
                view.getSimPhotoView().setImageDrawable(drawable);
            } else {
                view.getSimPhotoView().setImageResource(R.drawable.contact_phone_icon);
            }
    }
	//bird add by wucheng 20190322 begin
