...
public class BatchOperationService extends Service {
    ...
    public int onStartCommand(Intent intent, int flag, int startId) {
        ...
        /*
         * SPRD:
         * Bug 334280 when Import phone Contacts to SIM ,kill the process,the task will stop.
         * @{
         */
        ...
        switch (mode) {
            ...
            case MODE_START_BATCH_DELETE: {
                ...
                /**
                 * SPRD:bug646046 optimize performance about batch delete contacts
                 *
                 * @{
                 */
	...
	class BatchDeleteTask extends AsyncTask<Void,Integer,Integer> {
        ...
        public Integer doInBackground(Void... v) {
             ...
             * @{
             */
            mActivityManager.setSelfProtectStatus(ActivityManager.PROCESS_STATUS_MAINTAIN);
            /*
             * @}
             */
		...
		String title=mResources.getString(R.string.working_batch_delete);
               /*
                * SPRD:
                * Bug355247 Batch delete too slow,The progress bar set initial value
                * @{
                */
	    ...
	    public void onCancelled(Integer result) {
                    ...
                    mBuilder.setAutoCancel(true)
                            ...
                            .setContentIntent(
                                    PendingIntent.getActivity(mContext, mTag.hashCode(),
                                            identityIntent, PendingIntent.FLAG_CANCEL_CURRENT))
                            .build());
            if (mWakeLock != null ) {
             ...
             * @{
             */
            mActivityManager.setSelfProtectStatus(ActivityManager.PROCESS_STATUS_IDLE);
            /*
             * @}
             */
	    }
        ...
        public void onPostExecute(Integer i) {
            // SPRD: debug optimizing test log
            if (android.os.Debug.isDebugOptimizing())
                Log.v(TAG,
	...
	class BatchImportExportTask extends AsyncTask<Void,Integer,Integer> {
        ...
        public Integer doInBackground(Void... v) {
            ...
            if (Log.isIloggable()) {
                Log.startPerfTracking(Constants.PERFORMANCE_TAG
                        + String.format(": Start copying %d contacts from %s to %s", mMax,
            ...
            for (String key : mContactIds) {
                ...
                /*
                 * The patch of Wei has this operation but not in Sprdroid4.1
                 * TODO: check the effect. if(operations.size() <
                 * sTransactionSize) { continue; }
                 */
		...
		for (String mimeType:mimeTypes) {
		    ...
		    /**SPRD: Bug694927 The type of phone number cannot be same when copy phono contact with two numbers to sim {@*/
	    ...
	    public void onPostExecute(Integer i) {
	            ...
	            if (exception.mErrorCode == IccPBForMimetypeException.CAPACITY_FULL) {
	                if (Email.CONTENT_ITEM_TYPE.equals(exception.mMimeType)) {
	                    if (SimAccountType.ACCOUNT_TYPE.equals(mDestAccount.type)) {
	                        title = mResources.getString(R.string.contactSavedErrorToastSimEmailFull);
                        } else if (USimAccountType.ACCOUNT_TYPE.equals(mDestAccount.type)) {
                            title = mResources.getString(R.string.contactSavedErrorToastUsimEmailFull);
                        } else {
	                ...
	                mLastException instanceof SQLiteDiskIOException)
	        {
                ...
                mNotificationManager.notify(mTag, 0, mBuilder.setAutoCancel(true)
                    ...
                    .setProgress(0, 0, false)
                    .setContentIntent(
                            PendingIntent.getActivity(mContext, mTag.hashCode(), identityIntent, 0))
                    .build());
	        mTasks.remove(mTag);
	        if (mWakeLock != null ) {
                Log.i(TAG, "onPostExecute : release wake lock ");
                ...
                if (type != null) {
                    ...
                    if (isSimAccount(mDestAccount)) {
                        if (mEvenPhoneNumber) {
                            ...
                            type = String.valueOf(Phone.TYPE_OTHER);
                        }
                        mEvenPhoneNumber = !mEvenPhoneNumber;
                    } else {
                        if (type.equals(String
                                .valueOf(Phone.TYPE_OTHER))) {
                            type = String.valueOf(Phone.TYPE_OTHER);
                        }
                    }
                    /**
	...
	class BatchStarredTask extends AsyncTask<Void, Integer, Integer> {
        ...
        public BatchStarredTask(Context context, ArrayList<String> lookupKeys, String tag) {
            ...
            mCancelPendingIntent = PendingIntent.getActivity(mContext, tag.hashCode(), intent, PendingIntent.FLAG_UPDATE_CURRENT);
            mNotificationManager = (NotificationManager) mContext
                    .getSystemService(Context.NOTIFICATION_SERVICE);
            mBuilder = new NotificationCompat.Builder(mContext);
            mResources = mContext.getResources();
        ...
        public Integer doInBackground(Void... v) {
            ...
            for (String key : lookupKeys) {
                ...
                /*
                 * SPRD:
                 * Bug 353789 Deleted favorite contacts added by clicking the 'menu_add_favorite' item,
                 * add again in the same way, can't display in quick dialer.
                 * @{
                 */
                // Undemote the contact if necessary
                final Cursor c = resolver.query(Uri.withAppendedPath(Contacts.CONTENT_LOOKUP_URI, key), new String[] {Contacts._ID},
                        null, null, null);
