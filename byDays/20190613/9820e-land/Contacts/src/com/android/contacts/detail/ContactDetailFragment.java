...
public class ContactDetailFragment extends Fragment implements FragmentKeyListener,
    ...
    private static final String PHONE_TYPE_DATA = "vnd.android.cursor.item/phone_v2";
    private static final String EMAIL_TYPE_DATA = "vnd.android.cursor.item/email_v2";
    ...
    private ArrayList<ChoiceListItem> choices = new ArrayList<ChoiceListItem>(2);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        choices.add(new ChoiceListItem(ChoiceListItem.ID_TAKE_PHOTO, this.getString(R.string.launcherDialer)));
        choices.add(new ChoiceListItem(ChoiceListItem.ID_TAKE_PHOTO, this.getString(R.string.send_mms)));
        Context mContext = getActivity();
        adapter = new ArrayAdapter<ChoiceListItem>(mContext,
    ...
    protected void bindData() {
         ...
         * SPRD:
         *   Bug635756
         *   When edit a contact, all its numbers are deleted and saved, and the contact details screen is blank, no prompt.
         * @{
         */
        if (mAllEntries.size() > 1) {
    ...
    * */
    ...
    private void showListPopup(View anchorView, ListAdapter adapter,
        ...
        if(getActivity() != null && (getActivity() instanceof ContactDetailActivity)){
            final ContactDetailActivity detailActivity = (ContactDetailActivity) getActivity();
            if(detailActivity.getLeftSkView() != null){
                detailActivity.getLeftSkView().setText("");
                mPopup.setOnDismissListener(new PopupWindow.OnDismissListener() {
                    @Override
                    public void onDismiss() {
                        detailActivity.getLeftSkView().setText(R.string.softkey_option);
                    }
                });
            }
        }
    ...
    static class DetailViewEntry extends ViewEntry implements Collapsible<DetailViewEntry> {
        ...
        */
        ...
        public void clickCall(View clickedView, Listener fragmentListener, String number) {
            if (fragmentListener == null || clickedView == null || number == null) return;
            fragmentListener.onCallItemClicked(number);
        }
    }
    ...
    private final class ViewAdapter extends BaseAdapter {
        ...
        private View getHeaderEntryView(View convertView, ViewGroup parent) {
            ...
            if (favoritesStar != null) {
                ...
                */
                if (mContactData != null && mContactData.getAccount() != null
                        && (SimAccountType.ACCOUNT_TYPE.equals(mContactData.getAccount().type) ||
                        USimAccountType.ACCOUNT_TYPE.equals(mContactData.getAccount().type))) {
...
//				}
        ...
        private final OnClickListener mPrimaryActionClickListener = new OnClickListener() {
            ...
            public void onClick(View view) {
                 ...
                 *SPRD Bug645152 add related email function for Contacts of pikel4.4 {@
                 * */
                DetailViewEntry detailViewEntry = (DetailViewEntry) entry;
                if(PHONE_TYPE_DATA.equals(detailViewEntry.mimetype)){
                    ...
                    if (intent == null) return;
                    mListener.onItemClicked(intent);
                    //showListPopup(view, adapter, popupItemListener);
                }else if(EMAIL_TYPE_DATA.equals(detailViewEntry.mimetype)){
                    startActivity(detailViewEntry.intent);
                }
                /**
                 * @}
                 * */
        ...
        private final OnClickListener mThirdaryActionClickListener = new OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (entry == null || !(entry instanceof DetailViewEntry))
                    return;
                final DetailViewEntry detailViewEntry = (DetailViewEntry) entry;
                final Intent intent = detailViewEntry.thirdaryIntent;
                if (intent == null)
                    return;
                mListener.onItemClicked(intent);
            }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (entry == null) return;
        DetailViewEntry e = (DetailViewEntry)entry;
        /**
         * Bug953988 New card contacts and local contacts can not add email information {@
            */
        if (EMAIL_TYPE_DATA.equals(e.mimetype)) return;
        /**
         * @}
         */
        e.clickCall(view, mListener, e.data);
    }
            ...
            */
            ...
            case KeyEvent.KEYCODE_CALL: {
                ...
                if (index != -1) {
                    ...
                    if (getActivity() != null && (getActivity() instanceof ContactDetailActivity)) {
                        final ContactDetailActivity detailActivity = (ContactDetailActivity) getActivity();
                        if (EMAIL_TYPE_DATA.equals(entry.mimetype)) {
                            if (detailActivity.getCenterKeyView() != null) {
                                detailActivity.getCenterKeyView().setVisibility(View.GONE);
                                return true;
                            }
                        } else {
                            if (detailActivity.getCenterKeyView() != null) {
                                if (entry != null && !"".equals(entry)) {
                                    detailActivity.setmPhoneNumbers(entry.data);
                                }
                                detailActivity.getCenterKeyView().setVisibility(View.VISIBLE);
                    ...
                    entry.clickCall(mListView.getChildAt(index), mListener, entry.data);
                    /**
                     * @}
                     */
