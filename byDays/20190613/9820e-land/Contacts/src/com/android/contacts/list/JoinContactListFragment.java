...
public class JoinContactListFragment extends ContactEntryListFragment<JoinContactListAdapter> {
            ...
            switch (loader.getId()) {
                case DISPLAY_NAME_LOADER: {
                    if (data != null && data.moveToFirst()) {
                        /**
                        * SPRD:
                        *   add feature: only one contact, give prompt when join.
                        *
                        ...
                        * @{
                        */
                        mTitle = data.getString(0);
                        showTargetContactName(getString(R.string.contact_list_loading), false);
                        /**
                        * @}
                        */
                    }
                ...
                case JoinContactListAdapter.PARTITION_ALL_CONTACTS: {
                    ...
                    */
                    if(data != null){
                        if (data.getCount() > 0) {
                            showTargetContactName(mTitle, true);
                        } else {
                            showTargetContactName(getString(R.string.noJoinContactDataWith), false);
                        }
                        Cursor suggestionsCursor = ((JoinContactLoaderResult) data).suggestionCursor;
                        onContactListLoaded(suggestionsCursor, data);
                    }
    ...
    * @{
    */
    private void showTargetContactName(String displayName, boolean hasContact) {
        String blurb;
        Activity activity = getActivity();
        TextView blurbView = (TextView) activity.findViewById(R.id.join_contact_blurb);
        if (hasContact) {
    ...
    protected View inflateView(LayoutInflater inflater, ViewGroup container) {
        ...
        */
        if (UniverseUtils.UNIVERSEUI_SUPPORT){
            return inflater.inflate(R.layout.join_contact_picker_list_content_overlay, null);
        }else{
            return inflater.inflate(R.layout.join_contact_picker_list_content, null);
        }
        /**
