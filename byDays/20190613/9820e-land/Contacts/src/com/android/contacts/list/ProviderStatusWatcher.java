                ...
                if (cursor != null) {
                    try {
                        if (cursor.moveToFirst()) {
                            ...
                            // enum too.
							//add by maoyufeng 20190522 begin 
                            int status = cursor.getInt(0);
                            android.util.Log.d(TAG,"doInBackground status " +status);
                            mProviderStatus = new ProviderStatusWatcher.Status(
                                    status == 4 ? 0 : status, cursor.getString(1));
                            android.util.Log.d(TAG,"doInBackground mProviderStatus cursor.getInt(0)" +cursor.getInt(0) +" cursor.getString(1) " +cursor.getString(1));
                            //add by maoyufeng 20190522 end
							return true;
                        }
                    } finally {
                        cursor.close();
