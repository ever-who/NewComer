...
public class AggregationSuggestionEngine extends HandlerThread {
    ...
    private void loadAggregationSuggestions(Uri uri) {
        ...
        Cursor cursor = contentResolver.query(uri, new String[]{Contacts._ID}, Contacts.DISPLAY_ACCOUNT_TYPE +
                " not in ('sprd.com.android.account.sim','sprd.com.android.account.usim') " +
                " AND (" + RawContacts.SYNC2 + " NOT LIKE " + "'fdn%' OR " + RawContacts.SYNC2 + " IS NULL)", null, null);
        if (cursor == null) {
            return;
        }
        try {
