...
public class StaticLayout extends Layout {
    ...
    /* package */ void generate(CharSequence source, int bufStart, int bufEnd,
        ...
        mLineCount = 0;
        //mark
        //Log.d(TAG, "static layout generate() called with: source = [" + source + "]-" + source.getClass().getName() + " ,source instanceof spanned = [" + (source instanceof Spanned) + "], bufStart = [" + bufStart + "], bufEnd = [" + bufEnd + "], paint = [" + paint + "], outerWidth = [" + outerWidth + "], textDir = [" + textDir + "], spacingmult = [" + spacingmult + "], spacingadd = [" + spacingadd + "], includepad = [" + includepad + "], trackpad = [" + trackpad + "], ellipsizedWidth = [" + ellipsizedWidth + "], ellipsize = [" + ellipsize + "]");
        int v = 0;
        ...
        for (int paraStart = bufStart; paraStart <= bufEnd; paraStart = paraEnd) {
            ...
            if (spanned != null) {
                ...
                //Log.d(TAG, "generate: LeadingMarginSpan=" + Arrays.toString(sp));//mark
                ...
                //Log.d(TAG, " static layout generate: getParagraphSpans=" + Arrays.toString(chooseHt));//mark
...
//            Log.d(TAG, "generate: chs=" + Arrays.toString(chs) + ",widths=" + Arrays.toString(widths)
//                    + ",chdirs=" + Arrays.toString(chdirs) + ",dir=" + dir + ",easy=" + easy + ",paraStart="
//                    + paraStart + ",paraEnd=" + paraEnd);
            ...
            for (int spanStart = paraStart, spanEnd; spanStart < paraEnd; spanStart = spanEnd) {
                ...
                if (spanned == null) {
                    ...
                    Log.d(TAG, "generate: 3 spanEnd=" + spanEnd);
                    int spanLen = spanEnd - spanStart;
                ...
                Log.d(TAG, "generate: spanStart=" + spanStart + ",spanEnd=" + spanEnd + ",spanLen=" + (spanEnd - spanStart));
                for (int j = spanStart; j < spanEnd; j++) {
                    ...
                    if (w <= width || isSpaceOrTab) {
                        ...
                        //Log.d(TAG, "output 1: here=" + here + ",endPos=" + endPos+"paraEnd="+paraEnd);//mark
                        //mark by BIRD@hujingcheng 20190625 customize out positions for dial ui 
                        if (customizeStaticLayout()) {
                            int[] changing = changePositionBeforeOut(here, endPos, paraEnd);
                            if(changing!=null && changing.length==2){
                                here = changing[0];
                                endPos = changing[1];
                                //Log.d(TAG, "output customizeStaticLayout: here=" + here + ",endPos=" + endPos);//mark
                            }
                        }
						//mark by BIRD@hujingcheng 20190625 customize out positions for dial ui 
                        ...
                        //Log.d(TAG, "generate: mLineCount="+mLineCount+",mMaximumVisibleLineCount="+mMaximumVisibleLineCount);//mark
                    }
                }
            }
            //Log.d(TAG, "generate before output rest : paraEnd=" + paraEnd + ",here=" + here);//mark
            if (paraEnd != here && mLineCount < mMaximumVisibleLineCount) {
                ...
                //Log.e(TAG, "output rest " + here + " to " + paraEnd);
        ...
        if ((bufEnd == bufStart || source.charAt(bufEnd - 1) == CHAR_NEW_LINE) &&
            ...
            //Log.e(TAG, "output last " + bufEnd);//MARK ORIGIN
        ...
        if (where == TextUtils.TruncateAt.START) {
            ...
            if (mMaximumVisibleLineCount == 1) {
                ...
                for (right = len; right >= 0; right--) {
                    ...
                    // SPRD: Bug 405221 - To avoid Baidu Video crash.
                    int index = right - 1 + lineStart - widthStart;
                    if (index < 0)
                        continue;
    ...
    /**
     * add by hujingcheng for override
     *
     * @return
     */
     ...
     * @param end
     * @param paraWords
     * @return position after customize
     */
    public int[] changePositionBeforeOut(int start, int end, int paraWords) {
        return null;
    }
     ...
     * @return
     */
    public int getMaximumVisibleLineCount() {
        return mMaximumVisibleLineCount;
    }
