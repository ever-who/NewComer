...
/**
 *   create by hujingcheng 20190625 for dial ui in DialtactActivity
 *   change StaticLayout used by DynamicLayout to BirdStaticLayout , others copy from DynamicLayout no change
 */
public class BirdDynamicLayout extends Layout
{
    private static final int PRIORITY = 128;
    private static final int BLOCK_MINIMUM_CHARACTER_LENGTH = 400;
    private static final String TAG = "BirdDynamicLayout";

    /**
     * Make a layout for the specified text that will be updated as
     * the text is changed.
     */
    public BirdDynamicLayout(CharSequence base,
                         TextPaint paint,
     ...
     */
    public BirdDynamicLayout(CharSequence base, CharSequence display,
                         TextPaint paint,
                         int width, Alignment align,
                         float spacingmult, float spacingadd,
                         boolean includepad) {
    ...
    public BirdDynamicLayout(CharSequence base, CharSequence display,
        ...
        Log.d(TAG, "BirdDynamicLayout: enter");
    ...
    private void reflow(CharSequence s, int where, int before, int after) {
        ...
        if (reflowed == null) {
            reflowed = new BirdStaticLayout(null);//StaticLayout(null);
        } else {
            reflowed.prepare();
        }
    ...
    void updateBlocks(int startLine, int endLine, int newLineCount) {
        Log.d("TextView", "updateBlocks() called with: startLine = [" + startLine + "], endLine = [" + endLine + "], newLineCount = [" + newLineCount + "]"
                +",mBlockEndLines="+ Arrays.toString(mBlockEndLines)+",mNumberOfBlocks="+mNumberOfBlocks);
        if (mBlockEndLines == null) {
            createBlocks();
            return;
        }
    ...
    private static class ChangeWatcher implements TextWatcher, SpanWatcher {
        public ChangeWatcher(BirdDynamicLayout layout) {
            mLayout = new WeakReference<BirdDynamicLayout>(layout);
        }

        private void reflow(CharSequence s, int where, int before, int after) {
            BirdDynamicLayout ml = mLayout.get();
        ...
        private WeakReference<BirdDynamicLayout> mLayout;
    }
    ...
    private static StaticLayout sStaticLayout = new BirdStaticLayout(null);//StaticLayout(null);
