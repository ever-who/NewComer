...
void show_boot_logo(void)
{
	...
	if (error_flag == 0) {
#if defined(MTK_CARRIEREXPRESS_PACK)
		anim_show_logo(get_logo_index(true));
#else
...
{
    //get_bootlogo_flag();
    SLOGE("[libshowlogo: %s %d]show kernel logo, index = 38 \n",__FUNCTION__,__LINE__);
    if (MTK_LOG_ENABLE == 1) {
...
#else
		SLOGE("[libshowlogo: %s %d]show kernel logo, kernel_logo_position=%d \n",__FUNCTION__,__LINE__,kernel_logo_position);
/* add by BIRD@hujingcheng 20190829 custom kernel logo */
#if defined(BIRD_BOOT_SWITCH)
        char boot_switch_flag[PROPERTY_VALUE_MAX];
        property_get("ro.boot.bootswitch",boot_switch_flag,"");
        int custom_logo_index=kernel_logo_position;
        if(strlen(boot_switch_flag)!=0){
            char *p;
            char *out[3];
            p=strtok(boot_switch_flag,"_");
            int j=0;
            while(p!=NULL){
                *(out+j)=p;
                p=strtok(NULL,"_");
            ...
            SLOGE("[libshowlogo: %s %d]show kernel logo, out[0]=%s \n",__FUNCTION__,__LINE__,out[0]);
            int custom_kernel_logo= atoi(out[0]);
            if(custom_kernel_logo>0){
                custom_logo_index += custom_kernel_logo;
            }
        }
        SLOGE("[libshowlogo: %s %d]show kernel logo, custom_logo_index=%d \n",__FUNCTION__,__LINE__,custom_logo_index);
		anim_show_logo(custom_logo_index);//mark
#else
...
/* add by BIRD@hujingcheng read flag from proinfo 20190828 */
int read_from_proinfo(){
    const int PRODUCT_INFO_LID=61;
    ...
    //if(nvramstr == NULL){
        //SLOGE("[libshowlogo: %s %d] malloc fail nvramstr\n",__FUNCTION__,__LINE__);
        //free(nvramstr);
    //}

    char changed_boot_animation[PROPERTY_VALUE_MAX];
    char temp[PROPERTY_VALUE_MAX];
    property_get("service.nvram_init",changed_boot_animation,"");
    property_get("ro.boot.bootreason",temp,"");


    SLOGE("[libshowlogo: %s %d] service.nvram_init =%s,ro.boot.bootreason=%s\n",__FUNCTION__,__LINE__,changed_boot_animation,temp);

    F_ID fd=NVM_GetFileDesc(PRODUCT_INFO_LID,&pRecSize,&pRecNum,IsRead);
    if(fd.iFileDesc == -1){
        SLOGE("[libshowlogo: %s %d] open file fail \n",__FUNCTION__,__LINE__);
        return -1;
    }

    buff = (char *)malloc(size);
    if(buff==NULL){
        SLOGE("[libshowlogo: %s %d] malloc fail \n",__FUNCTION__,__LINE__);
        return -1;
    }
    ...
    if((ssize_t)size == read(fd.iFileDesc,buff,(ssize_t)size)){
		...
		{
			SLOGE("[libshowlogo: %s %d] read done \n",__FUNCTION__,__LINE__);
			//return buff;
   		}
		else
		{
			//NVRAM_LOG("Close file error!\n");
			SLOGE("[libshowlogo: %s %d] close file fail \n",__FUNCTION__,__LINE__);
			free(buff);
			//free(nvramstr);
			//delete[] cstr;
			return -1;
     		...
     		SLOGE("[libshowlogo: %s %d] read file fail \n",__FUNCTION__,__LINE__);
     		if(!NVM_CloseFileDesc(fd))
     			SLOGE("[libshowlogo: %s %d] close file fail \n",__FUNCTION__,__LINE__);
     		free(buff);
            //free(nvramstr);
    ...
    for(int i=0;i<(int)size;i++,buff++){
        //nvramptr += sprintf(nvramptr, "%02X",buff[i]);
        SLOGE("[libshowlogo] DONE buff[%d] = %02X\n",i,buff[i]);
    }

    //sprintf(nvramptr,"\n");
    //*(nvramptr+1)='\0';

   // SLOGE("[libshowlogo: %s %d] DONE nvramstr=%s\n",__FUNCTION__,__LINE__,nvramstr);
    free(buff);
    //free(nvramstr);
    return 1;
}
