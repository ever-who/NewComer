...
bird:BUG #40739 remove by lizhenye 20180910*/
...
bool BootAnimation::threadLoop()
{
	...
	ALOGD("WL_DEBUG threadLoop initAudioPath begin");
    const char* resourcePath = NULL;
	bool BIRD_BOOT_ANIMATION_SWITCH = property_get_bool("ro.bird.boot_animation_switch", false);
	bool bootAnimationSwitch = property_get_bool("persist.bird.boot_animation_switch", true);
	if (!BIRD_BOOT_ANIMATION_SWITCH || bootAnimationSwitch) {
		resourcePath = initAudioPath();
	}
	ALOGD("WL_DEBUG threadLoop initAudioPath end BIRD_BOOT_ANIMATION_SWITCH = %d, bootAnimationSwitch = %d", BIRD_BOOT_ANIMATION_SWITCH, bootAnimationSwitch);
...
const char* BootAnimation::initAudioPath() {
    ...
    if (bBootOrShutDown) {
        index = 0;
		bool firstTime = property_get_bool("persist.bird.boot_animation_first_time", true);
		ALOGD("WL_DEBUG firstTime = %d", firstTime);
		if (firstTime) {
			usleep(39000000);
			property_set("persist.bird.boot_animation_first_time", "false");
		}
    } else {
        index = 1;
    }
...
//mark by hujingcheng 20190828 for custom bootanimation by secret code
#ifdef BIRD_BOOT_SWITCH
        char changed_boot_animation[PROPERTY_VALUE_MAX];
        property_get("ro.boot.bootswitch",changed_boot_animation,"");
        char *p;
        char *out[3];
        if(strlen(changed_boot_animation)!=0){
            p=strtok(changed_boot_animation,"_");
            int j=0;
            while(p!=NULL){
                *(out+j)=p;
                p=strtok(NULL,"_");
        ...
        char CHANGED_BOOTANIMATION_FILE[PROPERTY_VALUE_MAX];
        sprintf(CHANGED_BOOTANIMATION_FILE,"/system/media/bootanimation_%s.zip",out[1]);
        ALOGD("regionalphone.db not OK, encryptedAnimation encryptedAnimation=%d, changed_boot_animation=%s,CHANGED_BOOTANIMATION_FILE=%s",encryptedAnimation,changed_boot_animation,CHANGED_BOOTANIMATION_FILE);//mark
#endif
//mark by hujingcheng 20190828 for custom bootanimation by secret code end
...
//mark by hujingcheng 20190828 for custom bootanimation by secret code
#ifdef BIRD_BOOT_SWITCH
            ((strlen(changed_boot_animation)!=0) && (access(CHANGED_BOOTANIMATION_FILE, R_OK) == 0) && (ZipFileName=CHANGED_BOOTANIMATION_FILE)
            &&((zipFile = ZipFileRO::open(CHANGED_BOOTANIMATION_FILE)) != NULL)) ||
#endif
//mark by hujingcheng 20190828 for custom bootanimation by secret code end
            ...
            ALOGD("regionalphone.db not OK, mZipFileName=%s",mZipFileName.string());//mark
        }
    }
}
