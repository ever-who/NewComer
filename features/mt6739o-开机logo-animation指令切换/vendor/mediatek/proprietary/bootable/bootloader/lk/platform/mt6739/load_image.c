...
#ifndef MTK_SECURITY_OEM_WITH_DMCERT_ONLY_SUPPORT
...
#else
	part_size = part->startblk * BLK_SIZE;
#endif
...
/* add by BIRD@hujingcheng 20190827 read from proinfo for nv flag */
#ifdef BIRD_BOOT_SWITCH
int get_bootlogo_flag(unsigned char *out,int size){
	dprintf(INFO, "mboot_get_boot_logo_flag enter\n");
	//unsigned long begin;
	long len,length;
    ...
    len = part_dev->read(part_dev,start_address,(uchar *)addr, length,part_id);
...
#endif
/* add by BIRD@hujingcheng 20190827 end */
...
{
	dprintf(INFO, "mboot_common_load_logo logo_addr=%ld , filename=%s\n", logo_addr, filename);//mark
	int ret;
	//int flag = get_bootlogo_flag();//mark
	//dprintf(INFO, "mboot_common_load_logo flag=%d\n", flag);//mark
	//if(flag >0){
...
#if (CONFIG_COMMANDS & CFG_CMD_FAT)
	dprintf(INFO, "mboot_common_load_logo CONFIG_COMMANDS & CFG_CMD_FAT\n");//mark
	len = file_fat_read(filename, (unsigned char *)logo_addr, 0);
	dprintf(INFO, "mboot_common_load_logo CONFIG_COMMANDS & CFG_CMD_FAT len=%d\n",(int)len);//mark
	if (len > 0)
		return (int)len;
#endif
 ...
 *
 **********************************************************/
static int mboot_android_check_bootimg_hdr(part_dev_t *dev, char *part_name, struct bootimg_hdr *boot_hdr)
{
	long len;
	int ret = 0;
	u64 addr;
 ...
 *
 **********************************************************/
static int mboot_android_check_recoveryimg_hdr(part_dev_t *dev, char *part_name, struct bootimg_hdr *boot_hdr)
{
	int ret = 0;
	long len;
	part_t *part;
...
#else
	start_addr = part->startblk * BLK_SIZE + g_boot_hdr->page_sz;
#endif
...
int mboot_android_load_recoveryimg(char *part_name, unsigned long addr)
{
	...
	len = partition_read(part_name, g_boot_hdr->page_sz, (uchar*)addr, (size_t)g_rcimg_sz);
...
unsigned int mboot_get_img_size(char *part_name, unsigned int *size)
{
	...
	*size = 0;
	...
	page_size = boot_hdr.page_sz;
	*size +=  page_size; /* boot header size is 1 page*/
	*size +=  (((boot_hdr.kernel_sz + page_size - 1) / page_size) * page_size);
	*size +=  (((boot_hdr.ramdisk_sz + page_size - 1) / page_size) * page_size);
	*size +=  (((boot_hdr.second_sz + page_size - 1) / page_size) * page_size);
...
int mboot_load_mntl(void)
{
	...
	*hdr_img_size = img_size;
	...
	if (img_auth_required) {
		...
		if (ret) {
			dprintf(CRITICAL, "[SBC] img %s vfy fail...(0x%x)\n", part_name, ret);
			assert(0);
		}
		dprintf(INFO, "[SBC] img vfy(%d ms)\n", (unsigned int)get_timer(img_vfy_time));
	} else
