...
//int get_bootlogo_flag();//mark
/*
 * ARGB8888 => RGB565
 *
 */
static unsigned short ARG8888_TO_RGB565(unsigned int data)
{
	unsigned int temp = data & 0xFFFFFF;
	unsigned short rgb = (unsigned short)(((temp & 0xf80000) >> 8) | ((temp & 0xfc00) >> 5) | ((temp & 0xf8) >> 3));
...
{
	int i = 0, j = 0;
	unsigned short *dst_addr = (unsigned short *)mt_get_boot_mode_addr();
	unsigned int *logo_decode_addr = (unsigned int *)mt_get_tempfb_addr();
	
	dprintf(INFO, "[lk logo: %s %d]\n",__FUNCTION__,__LINE__);
	mt_logo_get_custom_if();

	if (logo_cust_if->show_boot_logo) {
	   // dprintf(INFO, "[lk logo: %s %d] logo_cust_if->show_boot_logo 1 \n",__FUNCTION__,__LINE__);
		logo_cust_if->show_boot_logo();
	} else {
		///show_logo(0);
		init_fb_screen();

/*add by BIRD@hujingcheng 20190829 for first logo customize */
#ifdef BIRD_BOOT_SWITCH
		int index = BOOT_LOGO_INDEX;
		unsigned char flags[3];
		int flag=get_bootlogo_flag(flags,3);
		if(flag > 0){
		    int boot_logo_flag=(int)(flags[0]);
		    if( boot_logo_flag > 0){
		        index = KERNEL_LOGO_INDEX+boot_logo_flag*2;//38+2*flag
		    }
		}
	    dprintf(INFO, "[lk logo: %s %d] logo_cust_if->show_boot_logo 2 index =%d\n",__FUNCTION__,__LINE__,index);
		fill_animation_logo(index, mt_get_fb_addr(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
#else
        fill_animation_logo(BOOT_LOGO_INDEX, mt_get_fb_addr(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
#endif
/*add by BIRD@hujingcheng 20190829 for first logo customize */

        for(i = 0; i < CFG_DISPLAY_HEIGHT; ++i)
		{
			for(j = 0; j < CFG_DISPLAY_WIDTH; ++j)
			{
				*dst_addr = ARG8888_TO_RGB565(*logo_decode_addr);
				logo_decode_addr++;
				dst_addr++;
			}
		}
		SpiSendData(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT, (void *)mt_get_boot_mode_addr());
...
void mt_disp_show_battery_capacity(UINT32 capacity)
{
	...
	if (logo_cust_if->show_battery_capacity) {
		...
		printf("mycat: data format changed successful!\n");
		SpiSendData(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT, (void *)mt_get_boot_mode_addr());
