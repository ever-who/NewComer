...
/* bird open  by lihan for cts on 20180502*/ 
...
static void mboot_allocate_bootimg_from_mblock(struct bootimg_hdr *p_boot_hdr)
{
	...
	if ((dtb_kernel_addr_mb!=LK_DT_BASE)) {
		dprintf(CRITICAL, "Warning! compare memory_layout.h with bootimg about DT\n");
		assert(0);
	}
	if ((ramdisk_addr_mb!=LK_RAMDISK_BASE)||(ramdisk_sz_mb > LK_RAMDISK_MAX_SIZE)) {
		dprintf(CRITICAL, "Warning! compare memory_layout.h with bootimg about ramdisk\n");
		assert(0);
	}
#endif
...
int bldr_load_dtb(char *boot_load_partition)
{
	...
	dtb_overlay(g_fdt, DTB_MAX_SIZE);
		   ...
		   void *ramdisk, unsigned ramdisk_sz)
{
	...
	cmdline_append(tmpbuf);
/* add by BIRD@hujingcheng 20190829 for export flag to kernel by cmdline */
#ifdef BIRD_BOOT_SWITCH
    unsigned char flags[3];
	int flag = get_bootlogo_flag(flags,3);//mark
	if(flag > 0){
	        char boot_switch_buf[16];
	        dprintf(INFO, " boot_switch_buf: flag[0] = %u,flag[1]=%u,flag[2]=%u\n", flags[0],flags[1],flags[2]);
	    	sprintf(boot_switch_buf,"%u_%u_%u",flags[0],flags[1],flags[2]);
        	dprintf(INFO, "mt_boot flag: %d, boot_switch_buf: %s\n", flag,boot_switch_buf);//mark
        	snprintf(tmpbuf, TMPBUF_SIZE, "%s%s", "androidboot.bootswitch=", boot_switch_buf);
        	cmdline_append(tmpbuf);
	}
#endif
/* add by BIRD@hujingcheng 20190829 for export flag to kernel by cmdline */
...
// lcd show hardwareinfo begin by yangweisong @ 2017.02.13
#ifndef MACH_FPGA_NO_DISPLAY
	...
	snprintf(cmdline_tmpbuf, CMDLINE_TMP_CONCAT_SIZE, "lcm_id=%d", lcm_ata_check_id);
	printf("lcm_ata_check_id = %x\n", lcm_ata_check_id);
	cmdline_append(cmdline_tmpbuf);
#endif//LCM_ATA_CHECK
	...
	cmdline_append(cmdline_tmpbuf);
#endif
// lcd show hardwareinfo end by yangweisong @ 2017.02.13
